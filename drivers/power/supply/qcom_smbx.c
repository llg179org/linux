...
static int smb_init_cooling(struct smb_chip *chip)
{
	...
	/*
	 * cdev_id 0: this node has #cooling-cells = <2> and no indexed
	 * cooling devices, so the legacy (node, 0) identity is what the
	 * thermal zones' cooling-device phandles refer to.
	 */
	tcd = devm_thermal_of_cooling_device_register(chip->dev, 0,
