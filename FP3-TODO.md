...
`integration/7.1.3`. Those are the two branches that are **not** upstream-bound,
which is why a project TODO can sit in the kernel tree at all: it must never
appear in a `submit/7.1.3/*` series. Do not carry it onto any other branch.
...
branch and integration) is defined in
...
git for-each-ref --format='%(refname:short) %(objectname:short=12)' \
  ...
  'refs/remotes/fork/integration/7.1.3'
```
...
32. **The package lags `integration/7.1.3`.** After the camera-provenance rewrite
    the pinned `_commit` in `linux-fp3/APKBUILD` is no longer an ancestor of
    integration — it is on the `archive/integration-7.1.3-pre-camera-provenance`
    lineage. Nothing compiled changed, so no rebuild is warranted, but the next
    `_commit` bump starts from the current integration head, not from the pin.
    ☠️ GitHub serves a source tarball only while the commit is reachable from some
    ref, which is why that archive tag exists at all — check before trusting a
    pin:
