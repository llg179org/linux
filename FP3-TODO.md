...
property and puts a `watchdog@b017000` node in the board file rather than in the
SoC `.dtsi`. Both would be fatal in a `submit/` series. Both are fine here, and
the reason they are here rather than there is written into the commit message —
along with why there is deliberately no `ramoops` node (tried at `0x8ee00000` and
at `0xd0000000`; nothing survives a reset on this device, so it would cost 2 MB
and imply a post-mortem capability that does not exist).
